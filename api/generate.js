...
const FALLBACK_SYSTEM_PROMPT = [
  ...
  "Стиль: предметный, плотный, без воды. Нужны конкретные детали: масса, форма, упаковка, сценарий употребления, цена, каналы.",
...
async function callTextModel(messages, temperature) {
  ...
  const timeoutMs = Number(process.env.REQUEST_TIMEOUT_MS || 32000);
  const timeoutId =
  ...
  try {
    const response = await fetch(process.env.QWEN_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages,
        temperature: clamp(Number.isFinite(temperature) ? temperature : 0.55, 0, 1),
        top_p: 0.9,
    ...
    if (!response.ok) {
      ...
      console.error("[generate:text] bad response", response.status, text.slice(0, 500));
      return null;
    }
...
function normalizeDraft(rawDraft, input) {
  ...
  const productCore = normalizeProductCore(rawDraft.product_core, input, fallback);
...
function normalizeProductCore(rawCore, input, fallback) {
  const fb = fallback.product_core;

  return {
    one_liner: pickNonEmpty(
      sanitizeText(rawCore?.one_liner),
      fb.one_liner
    ),
...
function buildFallbackDraft(input) {
  ...
  const audience = pickNonEmpty(
    input.audience,
    audienceList.length ? audienceList.join(", ") : "Люди, которым нужно более умное решение привычной задачи"
  );
  ...
  Object.entries(BLOCK_SCHEMAS).forEach(([key, schema]) => {
    fallbackBlocks[key] = schema.map((item, index) => ({
      no: item.no,
      question: item.question,
      answer: fallbackAnswers[key][index] || `Нужно раскрыть вопрос: ${item.question}`
    }));
  });
  ...
  const productCore = {
    ...
    appearance: `Внешний вид: чистый современный дизайн, крупный маркер ключевой пользы, понятное цветовое кодирование вкусов или вариантов.`,
    ...
    usage: `Сценарий использования: человек быстро достаёт продукт, открывает без подготовки и использует в ситуации, где обычные решения неудобны.`,
    novelty_mechanism: `Новизна создаётся не ради новизны, а через более удачное сочетание формы, пользы, сценария использования и восприятия.`,
    why_people_will_try_it: `Продукт попробуют, потому что он обещает более удобный и конкретный результат, чем привычные альтернативы.`
  };
